...
the \ac{IXI} dataset\fnurl{https://brain-development.org/ixi-dataset/}.
...
Memory limitations often require
training and inference steps to be performed using
image subvolumes or \textit{patches} instead of the whole volumes,
as explained in \cref{sec:computation}.
%
In this section, we describe how TorchIO implements patch-based training via
image sampling and queueing.


\paragraph{Samplers}

A sampler takes as input an instance of \texttt{Subject} and returns a version
of it whose images have a reduced \ac{FOV}, i.e.,
the new images are subvolumes, also called windows or \textit{patches}.
%
For this, a \texttt{PatchSampler} may be used.


Different criteria may be used to select the center voxel of each output
patch.
%
...
the resulting volume patch by patch (or batch by batch).  % like bricks to build a house
% from each patch (or batch of patches).
...
The end of a training epoch is defined as the moment after which patches from
...
%
%A PyTorch loader queries the datasets copied in each subprocess,
%which load and process the volumes in parallel on the \ac{CPU}.
