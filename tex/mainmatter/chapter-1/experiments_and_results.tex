...
%assigning a weight four times larger to \acp{TCS} than \acp{FOS} to overcome dataset imbalance.
...
\begin{figure}[t!]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        \label{fig:betas}
    \end{subfigure}
    % \includegraphics[width=0.5\linewidth]{beta}
    \caption{%
...
%     \caption{
%         Effect of the shape parameter $\gamma$ on the probability distribution function used to sample snippets from each segment. The functions have been rescaled for visualization purposes.
%     }
%     \label{fig:betas}
% \end{figure}
...
Sampling beta:
    Uniform: beta 1
    Near semi-elliptic: 1.25
    Parabolic: 2
    Delta: $\infty$
Aggregations: mean, \ac{LSTM}, \ac{BLSTM}.
Num segments: 2, 4, 8, 16

4 * 5 * 3 = 60 trainings.
}

% % Sampling is the value of the parameter for the beta distribution
% % \setlength{\tabcolsep}{3pt}
% \begin{table}
%     \setlength{\tabcolsep}{2pt}
%     \centering
%     \caption{%
%         Best parameters and performance for each aggregation strategy (according to accuracy), averaging predictions from both views.
%     }
%     \label{tab:aggregation}
%     \begin{tabular}{l*7c}
%         \toprule
%         \textbf{Aggregation} & \textbf{Segments} & $\bm{\gamma}$ & \textbf{Accuracy} & \textbf{TP} & \textbf{TN} & \textbf{FP} & \textbf{FN} \\
%         \midrule
%         Mean                 &                 8 &             4 &              95.1 &          69 &         105 &           1 &           8 \\
%         \ac{LSTM}            &                 8 &             1 &              98.4 &          75 &         105 &           1 &           2 \\
%         \ac{BLSTM}           &                16 &             4 &              98.9 &          76 &         105 &           1 &           1 \\
%         \bottomrule
%     \end{tabular}
% \end{table}